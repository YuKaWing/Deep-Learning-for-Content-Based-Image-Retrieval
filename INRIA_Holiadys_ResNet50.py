...
path = os.path.sep.join([config.OUTPUT_PATH, "ResNet50_INRIA_Holidays_train_head.png"])
...
path = os.path.sep.join([config.OUTPUT_PATH, "ResNet50_INRIA_Holidays_train_model.png"])
callbacks = [TrainingMonitor(path)]
opt = SGD(lr=0.0001, momentum=0.9)
model.compile(loss="categorical_crossentropy", optimizer=opt, metrics=["accuracy"])
