...
import Config_oxbuild_images_3 as config
...
MODEL_PATH=os.path.sep.join([config.OUTPUT_PATH, "ResNet50_oxbuild_images.model"])
TRAINED_HEAD_MODEL_PATH=os.path.sep.join([config.OUTPUT_PATH, "ResNet50_oxbuild_images_trained_head"])
...
# path = os.path.sep.join([config.OUTPUT_PATH, "ResNet50_oxbuild_images_train_head(1).png"])
# callbacks = [TrainingMonitor1(path)]
# opt = SGD(lr=0.00001, momentum=0.9)
# model.compile(loss="categorical_crossentropy", optimizer=opt, metrics=["accuracy"])

# print("[INFO] training head...(1)")
# model.fit_generator(
#     trainGen.generator(),
#     steps_per_epoch=trainGen.numImages // batch_size,
#     validation_data=valGen.generator(),
