...
modelPath = os.path.sep.join([config.OUTPUT_PATH, "ResNet50_oxbuild_images_trained_head"])
...
for query in queries:
    ...
    queryFeature = model.predict(batchImages, batch_size=10)
    queryFeature = queryFeature.reshape((queryFeature.shape[0], 2048))
    
    searcher = Searcher(config.FEATURE_HDF5)
