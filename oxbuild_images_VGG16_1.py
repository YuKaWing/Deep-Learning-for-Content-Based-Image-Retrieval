...
path = os.path.sep.join([config.OUTPUT_PATH, "VGG16_oxbuild_images_train_head.png"])
...
path = os.path.sep.join([config.OUTPUT_PATH, "VGG16_oxbuild_images_train_model.png"])
callbacks = [TrainingMonitor(path)]
opt = SGD(lr=0.0001, momentum=0.9)
model.compile(loss="categorical_crossentropy", optimizer=opt, metrics=["accuracy"])
